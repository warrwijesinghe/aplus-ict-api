...
export const studentQuizLanding = async (userId, quizId) => db.sequelize.transaction(async (transaction) => {
  ...
  const questions = await buildAttemptQuestions(quiz, transaction);
  const questionCount = questions.length, totalMarks = precise(questions.reduce((sum, item) => sum + item.marks, 0));
  ...
  const graded = attempts.filter((item) => item.gradingStatus === "graded" && item.score !== null && item.percentage !== null);
  const selected = quiz.gradingMethod === "first" ? graded.at(-1) : quiz.gradingMethod === "latest" ? graded[0] : quiz.gradingMethod === "highest" ? graded.reduce((best, item) => !best || Number(item.percentage) > Number(best.percentage) ? item : best, null) : null;
  const finalMark = quiz.gradingMethod === "average" && graded.length ? (() => { const score = precise(graded.reduce((sum, item) => sum + Number(item.score), 0) / graded.length), maximumScore = precise(graded.reduce((sum, item) => sum + Number(item.maximumScore), 0) / graded.length), percentage = precise(graded.reduce((sum, item) => sum + Number(item.percentage), 0) / graded.length); return { score, maximumScore, percentage, passed: percentage >= Number(quiz.passPercentage) }; })() : selected ? { score: Number(selected.score), maximumScore: Number(selected.maximumScore), percentage: Number(selected.percentage), passed: selected.passed } : null;
  return { quiz: { id: quiz.id, title: quiz.title, description: quiz.description, instructions: quiz.instructions, availableFrom: quiz.availableFrom, availableUntil: quiz.availableUntil, timeLimitMinutes: quiz.timeLimitMinutes, attemptsAllowed: quiz.attemptsAllowed, passPercentage: Number(quiz.passPercentage), gradingMethod: quiz.gradingMethod, questionCount, totalMarks }, finalMark, attemptsUsed: attempts.length, remainingAttempts: Math.max(0, Number(quiz.attemptsAllowed || 0) - attempts.length), resumableAttemptId: active?.id || null, attempts: attempts.map(summary) };
});
