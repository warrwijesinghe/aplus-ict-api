...
export const submitAttempt = async (userId, attemptId) => db.sequelize.transaction(async (transaction) => { let attempt = await loadAttempt(attemptId, transaction, true); assertOwner(attempt, userId); if (completedStatuses.includes(attempt.status)) return serializeAttempt(attempt, { includeQuestions: false, includeFeedback: true }); const expired = Boolean(attempt.expiresAt && new Date(attempt.expiresAt) <= now()); await gradeAttempt(attempt, transaction, expired); attempt = await loadAttempt(attemptId, transaction); return serializeAttempt(attempt, { includeQuestions: false, includeFeedback: true }); });
